import moment from 'moment';

import postImage2 from '@assets/images/photo-of-people-doing-handshakes-3183197.png';
import postImage4 from '@assets/images/photo-of-people-walking-on-hallway-3182811.png';
import postImage1 from '@assets/images/photo-of-woman-looking-at-man-3183173.png';
import postImage from '@assets/images/photo-of-woman-wearing-eyeglasses-3184405.png';
import postImage3 from '@assets/images/two-women-in-front-of-dry-erase-board-1181533.png';
import Authors from '@constants/authors';
import { Categories, type IPost } from '@typing';

const postsEn: Record<string | number, IPost> = {
  '1': {
    title: 'Step-by-step guide to choosing great font pairs',
    category: Categories.BUSINESS,
    author: Authors['1'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
    tags: ['Business', 'Life'],
  },
  '2': {
    title: 'A UX Case Study Creating a Studious Environment for Students: ',
    category: Categories.TECHNOLOGY,
    author: Authors['2'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    tags: ['Business', 'Screen'],
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '3': {
    title: 'How to build rapport with your web design clients',
    category: Categories.STARTUP,
    author: Authors['3'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    tags: ['Business', 'Screen'],
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '4': {
    title: 'Font sizes in UI design: The complete guide to follow',
    category: Categories.STARTUP,
    author: Authors['4'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    tags: ['Business', 'Marketing'],
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '5': {
    tags: ['Screen', 'Experience'],
    title: '8 Figma design systems you can download for free today',
    category: Categories.BUSINESS,
    author: Authors['5'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '6': {
    title: 'Step-by-step guide to choosing great font pairs',
    category: Categories.BUSINESS,
    author: Authors['6'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
    tags: ['Business'],
  },
  '7': {
    tags: ['Screen', 'Experience'],
    title: 'A UX Case Study Creating a Studious Environment for Students: ',
    category: Categories.TECHNOLOGY,
    author: Authors['7'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '8': {
    tags: ['Screen', 'Experience'],
    title: 'How to build rapport with your web design clients',
    category: Categories.STARTUP,
    author: Authors['8'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '9': {
    tags: ['Screen', 'Experience'],
    title: 'Font sizes in UI design: The complete guide to follow',
    category: Categories.STARTUP,
    author: Authors['1'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '10': {
    tags: ['Screen', 'Experience'],
    title: '8 Figma design systems you can download for free today',
    category: Categories.BUSINESS,
    author: Authors['2'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '11': {
    tags: ['Screen', 'Experience', 'Technology'],
    title: 'Step-by-step guide to choosing great font pairs',
    category: Categories.BUSINESS,
    author: Authors['3'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '12': {
    tags: ['Screen', 'Experience', 'Technology'],
    title: 'A UX Case Study Creating a Studious Environment for Students: ',
    category: Categories.TECHNOLOGY,
    author: Authors['4'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '13': {
    tags: ['Screen', 'Experience', 'Technology'],
    title: 'How to build rapport with your web design clients',
    category: Categories.STARTUP,
    author: Authors['5'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '14': {
    tags: ['Screen', 'Experience', 'Technology', 'Life'],
    title: 'Font sizes in UI design: The complete guide to follow',
    category: Categories.STARTUP,
    author: Authors['6'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '15': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: '8 Figma design systems you can download for free today',
    category: Categories.BUSINESS,
    author: Authors['7'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '16': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: 'Step-by-step guide to choosing great font pairs',
    category: Categories.BUSINESS,
    author: Authors['8'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '17': {
    tags: [
      'Screen',
      'Experience',
      'Technology',
      'Life',
      'Marketing',
      'Business',
    ],
    title: 'A UX Case Study Creating a Studious Environment for Students: ',
    category: Categories.TECHNOLOGY,
    author: Authors['1'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '18': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: 'How to build rapport with your web design clients',
    category: Categories.STARTUP,
    author: Authors['2'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '19': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: 'Font sizes in UI design: The complete guide to follow',
    category: Categories.ECONOMY,
    author: Authors['3'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '20': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: '8 Figma design systems you can download for free today',
    category: Categories.ECONOMY,
    author: Authors['4'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
};

export const postsRu: Record<string | number, IPost> = {
  '1': {
    title: 'Шаг за шагом руководство по выбору отличных пар шрифтов',
    category: Categories.BUSINESS,
    author: Authors['1'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
    tags: ['Business', 'Life'],
  },
  '2': {
    title: 'Практический пример UX Создание учебной среды для студентов:',
    category: Categories.TECHNOLOGY,
    author: Authors['2'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
    tags: ['Business', 'Screen'],
  },
  '3': {
    title: 'Как наладить отношения с клиентами веб-дизайна',
    category: Categories.STARTUP,
    author: Authors['3'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    tags: ['Business', 'Screen'],
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '4': {
    title:
      'Размеры шрифтов в дизайне пользовательского интерфейса: полное руководство, которому нужно следовать',
    category: Categories.STARTUP,
    author: Authors['4'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    tags: ['Business', 'Marketing'],
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '5': {
    tags: ['Screen', 'Experience'],
    title:
      '8 дизайн-систем Figma, которые вы можете скачать бесплатно уже сегодня',
    category: Categories.BUSINESS,
    author: Authors['5'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '6': {
    title: 'Шаг за шагом руководство по выбору отличных пар шрифтов',
    category: Categories.BUSINESS,
    author: Authors['6'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
    tags: ['Business'],
  },
  '7': {
    tags: ['Screen', 'Experience'],
    title: 'Практический пример UX Создание учебной среды для студентов:',
    category: Categories.TECHNOLOGY,
    author: Authors['7'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '8': {
    tags: ['Screen', 'Experience'],
    title: 'Как наладить отношения с клиентами веб-дизайна',
    category: Categories.STARTUP,
    author: Authors['8'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '9': {
    title:
      'Размеры шрифтов в дизайне пользовательского интерфейса: полное руководство, которому нужно следовать',
    category: Categories.STARTUP,
    author: Authors['1'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '10': {
    tags: ['Screen', 'Experience'],
    title:
      '8 дизайн-систем Figma, которые вы можете скачать бесплатно уже сегодня',
    category: Categories.BUSINESS,
    author: Authors['2'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '11': {
    title: 'Шаг за шагом руководство по выбору отличных пар шрифтов',
    category: Categories.BUSINESS,
    author: Authors['3'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    tags: ['Screen', 'Experience', 'Technology'],
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '12': {
    tags: ['Screen', 'Experience', 'Technology'],
    title: 'Практический пример UX Создание учебной среды для студентов:',
    category: Categories.TECHNOLOGY,
    author: Authors['4'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '13': {
    tags: ['Screen', 'Experience', 'Technology'],
    title: 'Как наладить отношения с клиентами веб-дизайна',
    category: Categories.STARTUP,
    author: Authors['5'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '14': {
    tags: ['Screen', 'Experience', 'Technology', 'Life'],
    title:
      'Размеры шрифтов в дизайне пользовательского интерфейса: полное руководство, которому нужно следовать',
    category: Categories.STARTUP,
    author: Authors['6'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '15': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title:
      '8 дизайн-систем Figma, которые вы можете скачать бесплатно уже сегодня',
    category: Categories.BUSINESS,
    author: Authors['7'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '16': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: 'Шаг за шагом руководство по выбору отличных пар шрифтов',
    category: Categories.BUSINESS,
    author: Authors['8'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '17': {
    tags: [
      'Screen',
      'Experience',
      'Technology',
      'Life',
      'Marketing',
      'Business',
    ],
    title: 'Практический пример UX Создание учебной среды для студентов:',
    category: Categories.TECHNOLOGY,
    author: Authors['1'],
    date: moment('20130620', 'YYYYMMDD').toDate(),
    image: postImage1,
    views: 650,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '18': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title: 'Как наладить отношения с клиентами веб-дизайна',
    category: Categories.STARTUP,
    author: Authors['2'],
    date: moment('20140620', 'YYYYMMDD').toDate(),
    image: postImage2,
    views: 1150,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '19': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title:
      'Размеры шрифтов в дизайне пользовательского интерфейса: полное руководство, которому нужно следовать',
    category: Categories.ECONOMY,
    author: Authors['3'],
    date: moment('20100620', 'YYYYMMDD').toDate(),
    image: postImage3,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
  '20': {
    tags: ['Screen', 'Experience', 'Technology', 'Life', 'Marketing'],
    title:
      '8 дизайн-систем Figma, которые вы можете скачать бесплатно уже сегодня',
    category: Categories.ECONOMY,
    author: Authors['4'],
    date: moment('20120620', 'YYYYMMDD').toDate(),
    image: postImage4,
    views: 350,
    text: [
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod.',
      ],
      [
        'body1',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'head2',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
      [
        'body1',
        ' Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do\n' +
          '          eiusmod tempor incididunt ut labore et dolore magna aliqua. Non\n' +
          '          blandit massa enim nec. Scelerisque viverra mauris in aliquam sem. At\n' +
          '          risus viverra adipiscing at in tellus. Sociis natoque penatibus et\n' +
          '          magnis dis parturient montes. Ridiculus mus mauris vitae ultricies\n' +
          '          leo. Neque egestas congue quisque egestas diam. Risus in hendrerit\n' +
          '          gravida rutrum quisque non.',
      ],
    ],
  },
};

export default postsEn;
